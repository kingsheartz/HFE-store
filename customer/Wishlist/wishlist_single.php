    ...
    <ol class="breadcrumb breadcrumb1" style="background-color: #eaeded">
      ...
      <li>
        <a href="../Wishlist/wishlist.php">
          <span class="glyphicon glyphicon-file" aria-hidden="true"></span>Wish list
        </a>
      </li>
...
<div class="single-product-area" style="padding-top: 0px; background-color: #eaeded;padding-bottom: 0px;">
  ...
  <div class="container nopadding-margin" style="margin-left: 0px;width: 100%;padding: 0px;padding-bottom: 30px;">
    <div class="row" style="margin: 0px;padding: 0px;">
              ...
              <tr class="list_items_row" onclick="$('.wishlist_items_show').show();$('.settings_row_close').show();$('.wishlist_items_show_open').show();$('.settings_show').hide();$('.settings_row_open').hide();$('.wishlist_items_show_close').hide();">
                ...
                <td align="right">
                  <h4>
                    ...
                    <i class="fa fa-chevron-down wishlist_items_show_open" style="display: none;"></i>
                  </h4>
                </td>
              </tr>
              <tr class="settings_row"
                onclick="$('.settings_show').show();$('.settings_row_open').show();
                  $('.wishlist_items_show_close').show();$('.settings_row_close').hide();
                  $('.wishlist_items_show_open').hide();$('.wishlist_items_show').hide();">
                <td style="cursor: pointer;">
                  <h4>
      ...
      <!----------------------------------------------------------------------------------------------------------------------------->
      <!----------------------------------------------------------------------------------------------------------------------------->
      <div class="col-md-12 wishlist_items_show" style="margin:0px;padding: 0px; padding-left: 15px;padding-right: 15px;">
                                          ...
                                          <div style="width: 180px;margin-top: 5px;" class="tbl_wi<?= $wishlist_items_id ?>">
                                            <div class="product-quantity quantity buttons_added" style="justify-content: center;display: flex;text-align: center;align-items: center;position: relative;margin-right: 0px">
                                              <div class="product_img div-wrapper" style="padding: 0px; margin-top: 7px;margin-left: 15px;grid-gap: 10px;">
                                                <input
                                                  style="display:none"
                                                  id="check_s<?= $store_id . "i" . $product_description_id ?>" type="checkbox"
                                                  name="select_item">
...
<script type="text/javascript">
  ...
  function updateall_cart() {
    ...
    while ($row1 = $stmt1->fetch(PDO::FETCH_ASSOC)) {
      ...
      $sql2 = "select * from product inner join category on category.category_id=product.category_id
							...
							inner join product_details on product_description.product_description_id=product_details.product_description_id
							inner join store on store.store_id=product_details.store_id
							where product.sub_category_id=sub_category.sub_category_id and product.product_id=:product_description_id and product_details.store_id=:store_id order by product_description.product_description_id";
      $stmt2 = $pdo->prepare($sql2);
      $stmt2->execute(array(
        ':product_description_id' => $product_description_id,
        ':store_id' => $store_id
  ...
  function select_item_option(store_id, product_description_id, tmrp) {
    ...
    if (old_value == '0') { //your specific condition
      remove_item(store_id, product_id);
  ...
  function sub_item_all(store_id, product_id, tmrp) {
    ...
    if (parseInt($('#btn_s' + store_id + 'i' + product_description_id).html()) != 1) {
      ...
      remove_item(store_id, product_description_id);
    }
    total(store_id, product_description_id, mrp);
  }

  function add_item_all(store_id, product_id, tmrp) {
    var store_id = store_id;
    var product_description_id = product_description_id;
    var mrp = tmrp;
