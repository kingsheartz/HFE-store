...
if (isset($_GET['id'], $_SESSION['id'])) {
  ...
  if ($check->rowCount() > 0) {
    $viewedsql = $pdo->prepare(
      "UPDATE product_keys SET views = views + 1, date_of_preview = :dop
      WHERE product_description_id = " . $_GET['id']
    );
    $date = date("Y\-m\-d");
    ...
    $viewedsql = $pdo->prepare(
      "INSERT INTO product_keys (views, customer_id, product_description_id, date_of_preview)
...
$ratingstmt = $pdo->query(
  "SELECT AVG(product_keys.rating) AS avgrate FROM product_keys
  WHERE product_description_id = " . $_GET['id'] . "
  AND rating > 0 AND ordered_cnt > 0 AND review != '0'"
);
...
<script type="text/javascript">
  ...
  function check_store_select() {
    console.log("check_store_select() called");
...
<div
  ...
  style="display: none;z-index: 99;background-repeat: no-repeat;max-width: 100%px;background-color:white;">
</div>
                        ...
                        ?>
                            ...
                            ?>
                              ...
                              <div class=" col-md-1 col-sm-1" style="justify-content: right;padding:0px;">
                                <div
                                  class="btn btn-default btn-lg btn-flat product_wish"
                                  type="button"
                                  name="submit"
                                  ...
                                  <i
                                    style="color: #c50505;display: flex;align-items: center;justify-content: center;margin-left: 3px;"
                                    class="fas fa-heart fa-lg mr-2">
                                  </i>
                                </div>
                                <div
                                  onclick="copylink(<?= $_GET['id'] ?>,'single.php?id=<?= $_GET['id'] ?>')"
                                  class="btn btn-default btn-lg btn-flat product_share"
                                  type="button"
                                  name="submit"
                                  class="btn btn-primary btn-lg button"
                                  data-toggle="modal"
                                  ...
                                  <i
                                    style="color: #999898;display: flex;align-items: left;justify-content: left;margin-left: -8px;border-color: #e2e2e2;"
                                    class="fas fa-share fa-lg mr-2">
                                  </i>
                                </div>
                                <div
                                  id="view-single-img"
              ...
              <div class="container bottom-small-img" style="margin:0;">
                <div class="row" style="margin:0;margin-right:5px;margin-left:5px">
                  ...
                  //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////-->
                      ...
                      ----------------------------------------------------------------------------------------------------------------------------------------------------------->
                          <?php
                          $sqlfeatures_color = "SELECT * FROM product_description WHERE product_description.product_id=:product_id";
                          ...
                          $sqlfeatures_size = "SELECT * FROM product_description WHERE product_description.product_id=:product_id";
                          ...
                          ?>
                              ...
                              ?>
                              <div class="btn-group btn-group-toggle" data-toggle="buttons" onclick="location.href='../Product/single.php?id=<?= $rowfeatures_size['product_description_id'] ?>'">
                                <?php
                                ...
                                ?>
                                  <label class="btn btn-default text-center">
                                    <input
                                      type="radio"
                                      name="color_option"
                                      id="color_option_b1<?= $rowsize_name['size_name'] ?>"
                                      autocomplete="off">
                                    <span class="text-xl">
                ...
                ?>
                      <section id="customer_reviewed_already" style="margin-top:20px;">
                        <div class="div-wrapper" style="width:max-content">
                          ...
                          <p>
                            <?= $customer_firstnm . " " . $customer_lastnm ?>
                          </p>
                        </div>
                        <div class="div-wrapper" style="width:max-content">
                          <?php
                          ...
                          $customer_dor = explode("-", $customer_date_of_review);
                          $customer_date = $customer_dor[2] . "/" . $customer_dor[1] . "/" . substr($customer_dor[0], 2);
                          ?>
                          <p>
                            <?= $customer_date ?>
                          </p>
    ...
    <h4
      ...
      <span style="float: right;margin-right: 5px;margin-top: -4px;">
        <button
          ...
          <a href="../Product/products_viewall.php?category_id=<?= $cat_id ?>" style="color:<?= $c2 ?>;">View all</a>
        </button>
      </span>
    </h4>
    <div class="difcat " style="border-radius: 5px;">
      <span class="difhed"></span>
      <div class="difrow hidescroll" id="difrow<?= $row['product_description_id'] ?>" onscroll="scrolllisten('difrow<?= $row['product_description_id'] ?>');">
        <button
      ...
      ?>
        ...
        <div class="newproducts-w3agile" style="padding:0;padding-top:10px;">
          ...
          ?>
            ...
            <div class="difcat " style="border-radius: 5px;">
              <span class="difhed"></span>
              <div class="difrow hidescroll" id="difrow1<?= $row['product_description_id'] ?>" onscroll="scrolllisten('difrow1<?= $row['product_description_id'] ?>');">
                <button
                  class="left-arrow-btn-all shadow_all_none"
                  onclick="moveleft('difrow1<?= $row['product_description_id'] ?>')"
                  style="display: none;">
                  <i class="fas fa-chevron-left"></i>
                </button>
                <button class="right-arrow-btn-all shadow_all_none" onclick="moveright('difrow1<?= $row['product_description_id'] ?>')">
                  <i class="fas fa-chevron-right"></i>
                </button>
                <?php
                ...
                ?>
                  <div class="products-all-in-one" title="<?= $row['product_name'] ?>" onclick="location.href='../Product/single.php?id=<?= $row['product_description_id'] ?>'">
                    <div style="display: flex;justify-content: center;height: 200px;width:100%;background: white;text-align: center;">
                      <img class="image" align="middle" src="../../images/<?= $row['category_id'] ?>/<?= $row['product_description_id'] ?>.jpg">
                    </div>
                    <?php
